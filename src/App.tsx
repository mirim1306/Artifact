...
const App = () => {
  ...
  const renderContent = () => {
    ...
    if (navTab === 'mypage' && user) return <MyPage user={user} onBack={() => setNavTab('home')} onRegister={() => setNavTab('register')} />;
            ...
            <SliderContainer>
              <SliderTrack $currentIndex={sliderIndex}>
                {sliderItems.map((item, idx) => (
                  <SlideItem key={idx} onDoubleClick={() => setSelectedProject(item.projectRef)}>
